...
function insert_order($order_details) {
  ...
  // insert each computer part
  foreach($_SESSION['cart'] as $computer_part_id => $quantity) {
    $detail = get_computer_part_details($computer_part_id);
    $query = "delete from order_items where
              orderid = '".$orderid."' and computer_part_id = '".$computer_part_id."'";
    $result = $conn->query($query);
    $query = "insert into order_items values
              ('".$orderid."', '".$computer_part_id."', ".$detail['price'].", $quantity)";
    $result = $conn->query($query);
    if(!$result) {
      return false;
    }
